...
if __name__ == '__main__':
    ...
    import pickle
    with open(ros_root.get_path('dynamic-path-planner') +'/o3-pv2.pickle', 'rb') as pickle_file:     state_action = pickle.load(pickle_file);

    # state_action = np.load(ros_root.get_path('dynamic-path-planner') + '/o3.npy', allow_pickle=True)

    # get coordinates and angles
    waypoints, angles = convertor(state_action)

    # initialize node
    rospy.init_node('o3', anonymous=True)

    # publisher object publishing to command velocity
    pub = rospy.Publisher('/obstacle3/cmd_vel', Twist, queue_size=10)

    # initialize subscriber node to get robot's odometry
    sub = rospy.Subscriber('/obstacle3/odom', Odometry, callback_odom)
