...
if __name__ == '__main__':
    ...
    import pickle
    with open(ros_root.get_path('dynamic-path-planner') +'/waypoints-pv2.pickle', 'rb') as pickle_file:     state_action = pickle.load(pickle_file);
    #np.load(ros_root.get_path('dynamic-path-planner') + '/waypoints.npy', allow_pickle=True)
    ...
    pub = rospy.Publisher('/planner_robot/cmd_vel', Twist, queue_size=10)

    # initialize subscriber node to get robot's odometry
    sub = rospy.Subscriber('/planner_robot/odom', Odometry, callback_odom)
